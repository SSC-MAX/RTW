...
class watermark_model:
    def __init__(self, transform_model_path='models/transform_model.pth', tau_word=0.75, lamda=0.83):
    ...
    def watermark_detector(self, text, alpha=0.05):
        p = 0.5
        encodings = self.get_encodings(text)
        n = len(encodings)
        ones = sum(encodings)
        if n == 0:
            z = 0
